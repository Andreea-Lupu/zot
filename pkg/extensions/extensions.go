...
func EnableScrubExtension(config *config.Config, storeController storage.StoreController,
	...
	if config.Extensions.Scrub != nil &&
		*config.Extensions.Scrub.Enable {
		defaultScrubInterval, _ := time.ParseDuration("2h")

		if config.Extensions.Scrub.Interval < defaultScrubInterval {
			config.Extensions.Scrub.Interval = defaultScrubInterval
		...
		log.Info().Msg("Scrub config not provided, skipping Scrub config update")
	}
}
