...
func TestServeScrubExtension(t *testing.T) {
	...
	Convey("scrub explicitly enabled, but scrub interval parameter not set", t, func(c C) {
		port := GetFreePort()
		baseURL := GetBaseURL(port)
		logFile, err := ioutil.TempFile("", "zot-log*.txt")
		So(err, ShouldBeNil)
		...
		So(err, ShouldBeNil)
		So(string(data), ShouldContainSubstring,
			"\"Extensions\":{\"Search\":null,\"Sync\":null,\"Metrics\":null,\"Scrub\":{\"Enable\":true,\"Interval\":86400000000000}}") // nolint:lll
	...
	Convey("scrub implicitly enabled with scrub interval param set", t, func(c C) {
		port := GetFreePort()
		baseURL := GetBaseURL(port)
		logFile, err := ioutil.TempFile("", "zot-log*.txt")
		So(err, ShouldBeNil)
		...
		So(string(data), ShouldContainSubstring,
			"\"Extensions\":{\"Search\":null,\"Sync\":null,\"Metrics\":null,\"Scrub\":{\"Enable\":true,\"Interval\":3600000000000}") // nolint:lll
		So(string(data), ShouldContainSubstring, "executing scrub to check manifest/blob integrity")
		So(string(data), ShouldContainSubstring,
	...
	Convey("scrub explicitly disabled", t, func(c C) {
		...
		So(err, ShouldBeNil)
		So(string(data), ShouldContainSubstring,
			"\"Extensions\":{\"Search\":null,\"Sync\":null,\"Metrics\":null,\"Scrub\":{\"Enable\":false,\"Interval\":10800000000000}}") // nolint:lll
		So(string(data), ShouldContainSubstring, "Scrub config not provided, skipping Scrub config update")
